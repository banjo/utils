...
describe("file", () => {
    ...
    describe("deleteFile", () => {
        it("delete basic", () => {
            fs.writeFileSync(defaultFileName, content);
            FileKit.deleteFile(defaultFileName);
    ...
    describe("copyFile", () => {
        it("copy basic", () => {
            ...
            FileKit.copyFile(defaultFileName, newFileName);
        ...
        it("copy to non-existing directory should create directory", () => {
            ...
            FileKit.copyFile(defaultFileName, newFileName, { logError: true });
            expect(fs.existsSync(newFileName)).toBe(true);
        ...
        it("copy from non-existing file should trigger error", () => {
            const errorFn = vi.fn();
            FileKit.copyFile(defaultFileName, "file2.txt", { onError: errorFn });
            expect(errorFn).toHaveBeenCalled();
        });

        it("copy from non-existing directory should trigger error", () => {
            const errorFn = vi.fn();
            FileKit.copyFile("folder/file.txt", "file2.txt", { onError: errorFn });
            expect(errorFn).toHaveBeenCalled();
        });
    ...
    describe("copyDirectory", () => {
        it("copy basic", () => {
            ...
            FileKit.copyDirectory(newDirectory, `${newDirectory}2`);
        ...
        it("copy from non-existing directory should trigger error", () => {
            const errorFn = vi.fn();
            FileKit.copyDirectory("folder", "folder2", { onError: errorFn });
            expect(errorFn).toHaveBeenCalled();
        });
        ...
        it("copy to subdirectory of source should trigger error", () => {
            ...
            fs.mkdirSync(newDirectory);
            const errorFn = vi.fn();
            FileKit.copyDirectory(newDirectory, `${newDirectory}/folder2`, { onError: errorFn });
            expect(errorFn).toHaveBeenCalled();
        ...
        it("copy to itself should trigger error", () => {
            ...
            fs.mkdirSync(newDirectory);
            const errorFn = vi.fn();
            FileKit.copyDirectory(newDirectory, newDirectory, { onError: errorFn });
            expect(errorFn).toHaveBeenCalled();
        });
        ...
        it("copy to file extension should trigger error", () => {
            ...
            fs.mkdirSync(newDirectory);
            const errorFn = vi.fn();
            FileKit.copyDirectory(newDirectory, `${newDirectory}.txt`, { onError: errorFn });
            expect(errorFn).toHaveBeenCalled();
        });
    });
});
